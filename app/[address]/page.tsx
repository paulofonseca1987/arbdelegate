...
export default function DelegatePage() {
  ...
  const isValidAddress = /^0x[a-fA-F0-9]{40}$/i.test(addressFromUrl || '');
  const delegateAddress = isValidAddress ? addressFromUrl.toLowerCase() : null;
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const [metadataRes, currentRes] = await Promise.all([
        fetch(`/api/data?endpoint=metadata&address=${delegateAddress}`),
        fetch(`/api/data?endpoint=current&address=${delegateAddress}`),
      ]);
      ...
      if (!metadataRes.ok || !currentRes.ok) {
        if (metadataRes.status === 404 || currentRes.status === 404) {
          ...
          const progressRes = await fetch(`/api/sync/progress?address=${delegateAddress}`);
          if (progressRes.ok) {
      ...
      const timelineRes = await fetch(`/api/data?endpoint=timeline&address=${delegateAddress}`);
      ...
      try {
        const [votesRes, votesMetaRes] = await Promise.all([
          fetch(`/api/votes?endpoint=votes&address=${delegateAddress}`),
          fetch(`/api/votes?endpoint=metadata&address=${delegateAddress}`),
        ]);
  ...
  const handleSyncVotes = async () => {
    try {
      ...
      const response = await fetch(`/api/votes/sync?address=${delegateAddress}`, { method: "POST" });
      ...
      if (response.ok) {
        ...
        // Refresh votes data
        const votesRes = await fetch(`/api/votes?endpoint=votes&address=${delegateAddress}`);
        if (votesRes.ok) {
          const votesData = await votesRes.json();
          setVotes(votesData.votes || []);
        }
        const votesMetaRes = await fetch(`/api/votes?endpoint=metadata&address=${delegateAddress}`);
        if (votesMetaRes.ok) {
          const votesMeta = await votesMetaRes.json();
          setVotesMetadata(votesMeta);
        }
  ...
  const handleSync = async () => {
    try {
      ...
      fetch(`/api/sync?address=${delegateAddress}`, { method: "POST" })
        .then(async (response) => {
          const result = await response.json();
          if (response.ok) {
            setSyncStatus(
  ...
  const fetchSyncProgress = async () => {
    try {
      const response = await fetch(`/api/sync/progress?address=${delegateAddress}`);
      if (response.ok) {
  ...
  useEffect(() => {
    ...
    // Fetch config for token address and other settings
    ...
    const checkAndAutoSync = async () => {
      try {
        const metadataRes = await fetch(`/api/data?endpoint=metadata&address=${delegateAddress}`);
        if (metadataRes.ok) {
          ...
          if (now - metadata.lastSyncTimestamp > oneHour) {
            ...
            fetch(`/api/sync/background?address=${delegateAddress}`, {
              method: "POST",
              headers: {
                "X-Sync-Token":
